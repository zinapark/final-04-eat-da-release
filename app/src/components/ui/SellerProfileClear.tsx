import { SellerProfileClearProps } from '@/app/src/types';
import Link from 'next/link';

export default function SellerProfileClear({
  sellerId,
  sellerName,
  rating,
  reviewCount,
  profileImage,
  description,
}: SellerProfileClearProps) {
  return (
    <Link href={`/sellers/${sellerId}`}>
      <article className="flex items-start gap-2.5 self-stretch rounded-lg px-2.5 hover:bg-gray-50 transition-colors cursor-pointer">
        <img
          src={profileImage}
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <header className="flex flex-col gap-0">
            <h3 className="text-paragraph font-semibold text-gray-800">
              {sellerName}
            </h3>

            <div className="flex items-center gap-0.5 text-paragraph font-regular text-gray-700">
              ...
              <span className="text-paragraph-sm text-eatda-orange">
                {rating.toFixed(1)}
              </span>
